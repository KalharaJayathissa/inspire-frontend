...
const SubjectPage = () => {
    ...
    try {
      ...
      if (nicsToStore.length > 0) {
        ...
        toast({
          ...
          description: "No student NICs were found in the database. Manual entry is still available.",
          variant: "destructive",
        });
      }
  ...
  const handleUpdateMarks = async () => {
    ...
    try {
      ...
      toast({
        title: "Error",
        description: error.message || "Failed to update marks. Please try again.",
        variant: "destructive",
      });
    } finally {
      setIsSubmitting(false);
  ...
  const handleInlineSave = async (record: MarkRecord) => {
    ...
    try {
      ...
      const response = await updateMarks(record.student_id, Number(editingMarks), subjectId);
    ...
    try {
      ...
      if (response.deletedMarks || response.message) {
        ...
        if (response.remainingMarks && Array.isArray(response.remainingMarks)) {
          ...
          setMarksList(prevMarks => prevMarks.filter(mark => mark.id !== recordToDelete.id));
        ...
        throw new Error('Delete operation failed - no confirmation in response');
      }
    } catch (error: any) {
      console.error('❌ Delete failed:', error);
      toast({
        title: "Error",
        description: error.message || "Failed to delete marks. Please try again.",
        variant: "destructive",
      });
    } finally {
      setIsDeleting(null);
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
    ...
    if (e.key === 'Enter' || e.key === 'ArrowRight') {
      ...
      if (e.key === 'Enter' && !isSubmitting && studentId.trim() && marks !== '') {
        if (editingRecord) {
          handleUpdateMarks();
        } else {
          handleSubmitMarks();
  ...
  const filteredMarksList = marksList.filter(record => record.subject_id === currentSubjectId);
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="hidden md:flex items-center justify-between h-16">
            <div className="flex items-center space-x-4">
              ...
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <h1 className="text-lg font-bold text-white capitalize select-none">{subject} Marks Entry</h1>
                </div>
              </div>
            </div>
          ...
          <div className="md:hidden py-4 space-y-3">
            ...
            <div className="flex items-center space-x-3">
              ...
              <div>
                <h1 className="text-base font-bold text-white !text-white capitalize select-none">{subject} Marks Entry</h1>
              </div>
            </div>
          </div>
        </div>
          ...
          <CardContent className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-2">
                <Label htmlFor="studentId" className="text-sm font-medium text-gray-800 select-none">
                  Student ID (NIC)
                  {isLoadingNICs && <span className="text-xs text-gray-500 ml-2">(Loading NICs...)</span>}
                </Label>
                <div className="relative">
                  <User className="absolute left-3 top-1/2 transform -translate-y-1/2 w-4 h-4 text-gray-400 z-10" />
                  <Input
                    id="studentId"
                    placeholder={isLoadingNICs ? "Loading NICs..." : "Type NIC (numbers only, max 12 digits)..."}
                    value={studentId}
                  ...
                  {/* No results message */}
                  {showSuggestions && suggestions.length === 0 && !isLoadingNICs && studentId.trim().length > 0 && allStudentNICs.length > 0 && (
              ...
              <div className="space-y-2">
                <Label htmlFor="marks" className="text-sm font-medium text-gray-800 select-none">
                  Marks (0-100)
                </Label>
                <div className="relative">
              ...
              >
                {isSubmitting ? (
                  <div className="flex items-center space-x-2">
                    ...
                    <span>{editingRecord ? 'Updating...' : 'Submitting...'}</span>
                  </div>
                ) : (
                  editingRecord ? 'Update Marks' : 'Submit Marks'
                )}
              </Button>
            </div>
          </CardContent>
        ...
        <Card className="mb-4 sm:mb-6 bg-white border-gray-200 shadow-sm">
          <CardHeader>
            <CardTitle className="flex items-center justify-between text-black select-none">
              ...
              <Badge variant="secondary" className="bg-gray-100 text-black border-gray-300 select-none">
                Total Students: {filteredMarksList.length}
              </Badge>
            </CardTitle>
            <CardDescription className="text-gray-600 select-none">
              All marks for this subject. Table refreshes automatically after adding or updating marks.
            </CardDescription>
          </CardHeader>
            <CardContent>
              {isLoading ? (
                <div className="flex items-center justify-center py-8">
                  <div className="flex items-center space-x-2">
                    ...
                    <span className="text-gray-600">Loading existing marks...</span>
                  </div>
                </div>
              ) : (
                <>
                    ...
                    <Table>
                      <TableHeader>
                        <TableRow className="hover:bg-transparent">
                          <TableHead className="text-black font-semibold select-none">Student ID (NIC)</TableHead>
                          <TableHead className="text-black font-semibold select-none">Marks</TableHead>
                          <TableHead className="text-black font-semibold select-none">Percentage</TableHead>
                          <TableHead className="text-black font-semibold select-none">Added At</TableHead>
                          <TableHead className="text-right text-black font-semibold select-none">Actions</TableHead>
                        </TableRow>
                      </TableHeader>
                      <TableBody>
                        ...
                        filteredMarksList.map((record, index) => {
                          ...
                          return (
                          <TableRow key={`${record.student_id}-${record.id || index}`}>
                              ...
                              {isEditing ? (
                                <div className="flex items-center space-x-2">
                                  <Input
                                    ...
                                    onChange={(e) => setEditingMarks(e.target.value === '' ? '' : Number(e.target.value))}
                                    onFocus={(e) => {
                                    ...
                                    placeholder="0-100"
                                  />
                                  <span className="text-sm text-gray-600 select-none">/100</span>
                                </div>
                              ) : (
                                <Badge variant="outline" className="text-black border-gray-300 select-none">
                                  {record.marks || 0}/100
                                </Badge>
                              )}
                            </TableCell>
                                  ...
                                  <div className="w-12 h-2 bg-gray-200 rounded-full">
                                    <div 
                                      className={`h-full rounded-full ${
                                        ((Number(editingMarks) || 0) / 100 * 100) >= 75 ? 'bg-green-500' : 
                                        ((Number(editingMarks) || 0) / 100 * 100) >= 50 ? 'bg-yellow-500' : 'bg-red-500'
                                      }`}
                                      style={{ width: `${Math.min(((Number(editingMarks) || 0) / 100 * 100), 100)}%` }}
                                    ></div>
                                  </div>
                                  <span className="text-sm text-muted-foreground select-none">
                                    {Math.round((Number(editingMarks) || 0) / 100 * 100)}%
                                  </span>
                                </div>
                              ) : (
                                <div className="flex items-center space-x-2">
                                  <div className="w-12 h-2 bg-gray-200 rounded-full">
                                    <div 
                                      className={`h-full rounded-full ${
                                        ((record.marks || 0) / 100 * 100) >= 75 ? 'bg-green-500' : 
                                        ((record.marks || 0) / 100 * 100) >= 50 ? 'bg-yellow-500' : 'bg-red-500'
                                      }`}
                                      style={{ width: `${Math.min(((record.marks || 0) / 100 * 100), 100)}%` }}
                                    ></div>
                                  </div>
                                  <span className="text-sm text-black select-none">
                                    {Math.round((record.marks || 0) / 100 * 100)}%
                                  </span>
                                </div>
                              )}
                            </TableCell>
                        ...
                        <TableRow>
                          <TableCell colSpan={5} className="text-center py-8 text-black select-none">
                            No marks found. Submit marks above to see them in this table.
                          </TableCell>
                        </TableRow>
                      )}
                    </TableBody>
                  ...
                  <div className="md:hidden space-y-3">
                    {filteredMarksList.length > 0 ? (
                      filteredMarksList.map((record, index) => {
                        ...
                        return (
                          <div key={`mobile-${record.student_id}-${record.id || index}`} className="bg-white border border-gray-200 rounded-lg p-4 shadow-sm hover:shadow-md transition-shadow">
                            ...
                            <div className="flex items-center justify-between mb-3 pb-3 border-b border-gray-200">
                              <div className="flex items-center space-x-2">
                                ...
                                <div>
                                  ...
                                  <h3 className="font-medium text-black">{record.student_id || 'N/A'}</h3>
                                </div>
                              </div>
                              <div className="flex items-center space-x-2">
                                {isEditing ? (
                            ...
                            <div className="bg-gray-50 rounded-lg p-3 border border-gray-200">
                              ...
                              <div className="flex items-center justify-between">
                                <div className="flex items-center space-x-2">
                                  ...
                                  <span className="text-sm font-medium text-black">Marks:</span>
                                </div>
                                {isEditing ? (
                                  <div className="flex items-center space-x-2">
                                    <Input
                              ...
                              <div className="space-y-1">
                                <div className="flex items-center justify-between">
                                  ...
                                  <span className="text-sm text-black">
                                    {isEditing ? (Number(editingMarks) || 0) : (record.marks || 0)}%
                                  </span>
                                </div>
                                <div className="w-full h-2 bg-gray-200 rounded-full">
                                  <div 
                                    className={`h-full rounded-full ${
                                      isEditing 
                                        ? ((Number(editingMarks) || 0) >= 75 ? 'bg-black' : 
                                           (Number(editingMarks) || 0) >= 50 ? 'bg-gray-600' : 'bg-gray-400')
                                        : ((record.marks || 0) >= 75 ? 'bg-black' : 
                                           (record.marks || 0) >= 50 ? 'bg-gray-600' : 'bg-gray-400')
                                    }`}
                                    style={{ 
                                      width: `${Math.min(isEditing ? (Number(editingMarks) || 0) : (record.marks || 0), 100)}%` 
                                    }}
                                  ></div>
                                </div>
                              </div>
                              ...
                              {/* Date */}
                              <div className="flex items-center justify-between">
                                <span className="text-sm font-medium text-black">Added:</span>
                                <span className="text-sm text-black">
        ...
        <Card className="bg-gray-50 border-gray-300">
          ...
          <CardContent className="space-y-4">
            <div className="bg-white rounded-lg p-4 border border-gray-300">
              <h4 className="font-semibold text-black mb-2 select-none">General Guidelines:</h4>
            ...
            {(subject?.toLowerCase() === 'c.math' || subject?.toLowerCase() === 'combined maths') && (
              <div className="bg-white rounded-lg p-4 border border-gray-300">
                <p className="text-sm text-gray-700">
                  <strong>Combined Mathematics (Code: 2):</strong> Ensure problem-solving steps are properly evaluated
                </p>
              </div>
            )}
          </CardContent>
      ...
      <AlertDialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Are you sure you want to delete marks for student <strong>{recordToDelete?.student_id}</strong>?
              <br />
              <br />
